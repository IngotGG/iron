...
class SqliteEngine<T: Any>(
        ...
        return iron.transaction {
            return@transaction entities.map { entity ->
                ...
                if (fetch) {
                    ...
                    prepare("SELECT * FROM ${controller.tableName} WHERE ${selector.toString(this@SqliteEngine)}", selector.params())
    ...
    override suspend fun update(entity: T, fetch: Boolean): T {
        ...
        return iron.transaction {
            prepare(
                "UPDATE ${controller.tableName} SET $columns WHERE ${selector.toString(this@SqliteEngine)}",
                selector.params(), entity
            )

            if (fetch) {
                prepare("SELECT * FROM ${controller.tableName} WHERE ${selector.toString(this@SqliteEngine)}", selector.params(), entity)
                    .single(controller.clazz.kotlin)
            } else {
                entity
            }
